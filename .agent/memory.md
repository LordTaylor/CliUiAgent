...
- **Hard Break (Loop Mitigation)**: Implementacja mechanizmu "Hard Break" (zatrzymanie po 2 identycznych odpowiedziach) jest kluczowa dla UX. Zapobiega to długotrwałemu zapętleniu agenta, które wcześniej mogło trwać do 25 iteracji.
- **Persistent Fingerprints**: Utrzymywanie sygnatur wywołań narzędzi (`tool:param`) między turami (inter-turn persistence) pozwala wykryć pętle, które nie są widoczne w pojedynczej odpowiedzi LLM (np. naprzemienne czytanie i błędy kompilacji).
- **Opt-in CoVe**: Chain-of-Verification (weryfikacja faktów) powinna być selektywna. Wyciszenie jej dla prostych zadań i pozostawienie dla ról wysokiego ryzyka (Architect/Reviewer) optymalizuje czas odpowiedzi i zużycie tokenów.
